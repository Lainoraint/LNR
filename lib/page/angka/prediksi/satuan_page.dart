...
class _SatuanPageState extends State<SatuanPage> {
  ...
  Future<void> _loadModel() async {
    try {
      _interpreter = await Interpreter.fromAsset(
          'assets/models/model_angka_satuan.tflite');
      setState(() {
        _isModelLoaded = true;
      });
    } catch (e) {
  ...
  Future<void> _recognizeDrawing() async {
    ...
    try {
      ...
      // Create output tensor for 10 classes
      var outputTensor = List.generate(1, (_) => List.filled(10, 0.0));
      ...
      for (int i = 0; i < results.length; i++) {
        ...
        if (probability > 0.10) {
          predictions.add({"label": i.toString(), "probability": probability});
        }
      }
